...
# som結果の読み込み
df_som = pd.read_csv("../som/result_som/som_torus_34_59_20231108_063848.csv")
vertices = df_som.iloc[:, :3].values
som = df_som.iloc[:, 3:].values
...
def som_to_gird():
    ...
    for i in features_index[:]:
        ...
        # 3. もし、2ですでに特徴量が配置されていたら、類似度が高い方に書き換える。
        if flat[max_index // n][max_index % n] != -1:
            # 類似度が高い方に書き換える
            if max_similarity > calc_cosine_similarity(features[flat[max_index // n][max_index % n]], som[max_index]):
                features_index.append(flat[max_index // n][max_index % n])
                flat[max_index // n][max_index % n] = i
    ...
    if len(not_placed_features_index) != 0:
        ...
        for i in not_placed_features_index:
            ...
            for x in range(m):
                for y in range(n):
                    if flat[x][y] == -1:
                        similarity = calc_cosine_similarity(features[i], som[x * n + y])
...
filename = '../som/result_som_list/result_SOM_torus_34_59_' + now.strftime('%Y%m%d_%H%M%S') + '.csv'
...
#%%
img_name = '../som/result_image/result_SOM_image_torus_34_59_' + now.strftime('%Y%m%d_%H%M%S') + '.png'
